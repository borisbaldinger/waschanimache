<?php
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		$fkey = 'fbthu_add_metaog';
		if(isset($_POST[$fkey])){$d = stripslashes($_POST[$fkey]);$d = $d=='on'?'':-1;}else{ $d=-1; }
		update_option('fbthu_add_metaog',$d);
		
		$fkey = 'fbthu_add_metalocale';
		if(isset($_POST[$fkey])){$d = stripslashes($_POST[$fkey]);$d = $d=='on'?'':-1;}else{ $d=-1; }
		update_option($fkey,$d);
		
		$fkey = 'fbthu_add_metadesc';
		if(isset($_POST[$fkey])){
			$fval = $_POST[$fkey]=='on' ? 1 : 0;
		} else {
			$fval = 0;
		}
		update_option($fkey,$fval);
	...
	function fbthu_options_page() {
		...
		<fieldset class="options">
			...
			<br />
			<br />
			<?php $chkd = get_settings('fbthu_add_metaog')?'':'checked'; ?>
			<label><input name="fbthu_add_metaog" type="checkbox" <?php echo $chkd; ?> />&nbsp; Add Meta og:url tag</label>
			
			<br />
			<?php $chkd = get_settings('fbthu_add_metalocale')?'':'checked'; ?>
			<label><input name="fbthu_add_metalocale" type="checkbox" <?php echo $chkd; ?> />&nbsp; Add Meta og:locale</label>
			<br />
			<?php $chkd = get_settings('fbthu_add_metadesc')?'checked':''; ?>
			<label><input name="fbthu_add_metadesc" type="checkbox" <?php echo $chkd; ?> />&nbsp; Add Meta og:description</label>
			<br />
