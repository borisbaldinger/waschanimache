...
class Locate_Map
{
	...
	function get_version() {
		$version = __('Version', 'liquidfolio') . ":" . self::$version;
		return $version;
	}
	...
	function ajax_map_save() {
		...
		if (!$mapdata)
			$this->ajax_response(__('Internal error, map was missing.  Your data has not been saved!', 'liquidfolio'));

		$map = new Locate_Api_Map($mapdata);
		$res = $map->save($postid);
		
		if ($res === false) {
			$this->ajax_response(__('Internal error - unable to save map.  Your data has not been saved!', 'liquidfolio'));
		} else {
	...
	function ajax_map_delete() {
		...
		if (Locate_Api_Map::delete($mapid) === false) {
			$this->ajax_response("Internal error when deleting map ID '$mapid'!");
		} else {
	...
	function ajax_map_create() {
		...
		$map->title = __('Untitled', 'liquidfolio');
