...
class Locate_Api_Poi extends Locate_Api
{
	...
	*/
	function geocode($auto=true) {
		...
		if (!$response)
			return new WP_Error('geocode', sprintf(__('No geocoding response from Google: %s', 'liquidfolio'), $response));
		...
		if ($status != 'OK')
			return new WP_Error('geocode', sprintf(__("Google cannot geocode address: %s, status: %s", 'liquidfolio'), $this->address, $status));

		if (!$response  || !isset($response->results) || empty($response->results[0]) || !isset($response->results[0]))
			return new WP_Error('geocode', sprintf(__("No geocoding result for address: %s", 'liquidfolio'), $this->address));
