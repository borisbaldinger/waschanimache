...
//			$html .= apply_filters('location_map_directions_html', null, $this, $options);
				...
				<div id='location_map_add_panel' style='visibility:hidden'>
					<p>
						<span class='submit' style='padding: 0; float: none' >
							<input class='button-primary' type='button' id='location_map_add_btn' value='<?php _e('Add', 'liquidfolio'); ?>' />
						</span>

						<span  id='location_map_add_address'>
							<b><?php _e('Location', 'liquidfolio') ?>: </b>
					...
					<td valign="top">
						<div id='location_map_left_panel'>
							<div id='location_map_maplist_panel'>
								<p>
									<b><?php _e('Current Maps', 'liquidfolio') ?></b>
									<input class='button-primary' type='button' id='location_map_create_btn' value='<?php _e('New Map', 'liquidfolio') ?>' />
								</p>
							...
							<div id='location_map_adjust_panel' style='display:none'>
								<div id='location_map_adjust'>
									<p>
										<b><?php _e('Title', 'liquidfolio') ?>: </b><input id='location_map_title' type='text' size='20' />
									</p>
									<p class='submit' style='padding: 0; float: none' >
										<input class='button-primary' type='button' id='location_map_save_btn' value='<?php _e('Save', 'liquidfolio'); ?>' />
										<input type='button' id='location_map_recenter_btn' value='<?php _e('Center', 'liquidfolio'); ?>' />
									</p>
									<hr/>
								</div>
	...
	function _localize()
	{
		...
		return array(
			'maps_in_post' => __('Maps in this post', 'liquidfolio'),
			'no_maps_in_post' => __('There are no maps yet for this post', 'liquidfolio'),
			'create_map' => __('Create a new map', 'liquidfolio'),
			'map_id' => __('Map ID', 'liquidfolio'),
			'untitled' => __('Untitled', 'liquidfolio'),
			'dir_not_found' => __('The starting or ending address could not be found.', 'liquidfolio'),
			'dir_zero_results' => __('Google cannot return directions between those addresses.  There is no route between them or the routing information is not available.', 'liquidfolio'),
			'dir_default' => __('Unknown error, unable to return directions.  Status code = ', 'liquidfolio'),
			'enter_address' => __('Enter address', 'liquidfolio'),
			'no_address' => __('No matching address', 'liquidfolio'),
			'did_you_mean' => __('Did you mean: ', 'liquidfolio'),
			'directions' => __('Directions', 'liquidfolio'),
			'edit' => __('Edit', 'liquidfolio'),
			'save' => __('Save', 'liquidfolio'),
			'cancel' => __('Cancel', 'liquidfolio'),
			'del' => __('Delete', 'liquidfolio'),
			'view' => __('View', 'liquidfolio'),
			'back' => __('Back', 'liquidfolio'),
			'insert_into_post' => __('Insert into post', 'liquidfolio'),
			'select_a_map' => __('Select a map', 'liquidfolio'),
			'title' => __('Title', 'liquidfolio'),
			'delete_prompt' => __('Delete this map marker?', 'liquidfolio'),
			'delete_map_prompt' => __('Delete this map?', 'liquidfolio'),
			'del' => __('Delete', 'liquidfolio'),
			'map_saved' => __('Map saved', 'liquidfolio'),
			'map_deleted' => __('Map deleted', 'liquidfolio'),
			'ajax_error' => __('Error: AJAX failed!  ', 'liquidfolio'),
			'click_and_drag' => __('Click & drag to move this marker', 'liquidfolio'),
			'zoom' => __('Zoom', 'liquidfolio'),
			'traffic' => __('Traffic', 'liquidfolio'),
			'standard_icons' => __('Standard icons', 'liquidfolio'),
			'my_icons' => __('My icons', 'liquidfolio')
		);
	}
	...
	function _directions_html($html, $map, $options)
	{
		...
		$html = "
			<div id='{$options->mapName}_directions' class='location-map-directions'>
				<form action=''>
			<p><span id='{$options->mapName}_car_button' class='location-map-car-button location-map-travelmode selected' title='" . __('By car', 'liquidfolio') . "' ></span><span id='{$options->mapName}_walk_button' class='location-map-walk-button location-map-travelmode' title='" . __('Walking', 'liquidfolio') . "' ></span><span id='{$options->mapName}_bike_button' class='location-map-bike-button location-map-travelmode' title='" . __('Bicycling', 'liquidfolio') . "' ></span></p>
			<p><span class='location-map-a' title='" . __('Start', 'liquidfolio') . "'></span><input  type='text' class='location-map-txt' id='{$options->mapName}_saddr' value='' /></p>
						<p><span class='location-map-swap' id='{$options->mapName}_addrswap' title='" . __('Swap start and end', 'liquidfolio') . "'></span></p>
						<p><span class='location-map-b' title='" . __('End', 'liquidfolio') . "'></span><input   class='location-map-txt'  type='text' id='{$options->mapName}_daddr' value='' /></p>
						<p><input type='submit' class='location-map-button liquidfolio_button' value='" . __('Get Directions', 'liquidfolio') . "' id='{$options->mapName}_get_directions' /><input type='button' class='location-map-button liquidfolio_button' value='" . __('Print Directions', 'liquidfolio') . "' id='{$options->mapName}_print_directions' /><input type='button' class='location-map-button liquidfolio_button' value ='" . __('Close', 'liquidfolio') . "' id='{$options->mapName}_close_directions' />
					</p>
				</form>
