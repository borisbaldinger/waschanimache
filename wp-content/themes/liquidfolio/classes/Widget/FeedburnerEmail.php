...
class Widget_FeedburnerEmail extends Widget_Default
{
	...
	function widget( $args, $instance ) {
        ...
        <form   action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $name ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
			<fieldset>
				...
				<input type="text"  name="email" id="email<?php global $wid; echo $wid ?>" placeholder="<?php _e('Your E-Mail', 'liquidfolio'); ?>" /><input type="hidden" value="<?php echo $name ?>" name="uri"/><input type="hidden" name="loc" value="en_US"/><button type="submit"><?php _e('submit email address', 'liquidfolio'); ?></button>
			</fieldset>
        </form>
		 <?php
		echo $after_widget;
	...
	function form( $instance ) {
		...
		// Defaults
		$defaults = array( 'title' => __( 'Sign up for our Newsletter', 'liquidfolio' ),'description' => __('Sign up for our mailing list to stay updated on the latest themes from the Queldorei!', 'liquidfolio'), 'feedname' => 'queldorei');
		...
		<div>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'description' ); ?>"><?php _e( 'Description:', 'liquidfolio' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'feedname' ); ?>"><?php _e( 'Write only feedburner name:', 'liquidfolio' ); ?></label>
