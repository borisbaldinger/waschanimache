...
class Widget_Twitter_Feed
{
	...
	protected function getTwitterMessages($username, $num = 5)
	{
		...
		$aMessage = fetch_feed('http://api.twitter.com/1/statuses/user_timeline.rss?screen_name=' . $username);
	...
	protected function getMessageTimestamp($publishDate)
	{
		...
		{
			$h_time = sprintf(__('%s ago', 'liquidfolio'), human_time_diff($time));
		}
		else
		{
			$h_time = date(__('Y/m/d', 'liquidfolio'), $time);
		}
		return sprintf(__('%s', 'liquidfolio'), ' <span class="twitter-timestamp">' . $h_time . '</span>');
	}
