...
class Widget_PopularPosts extends Widget_Default implements Widget_Interface_Cache
{
					...
					<?php if($current_post_id != get_the_ID() && $shown_post <= $instance[self::NUMBER]):?>
						<li>
							...
							<div class="recent_txt">
								...
								<?php if (comments_open()) { echo "<p>"; comments_popup_link(__('0 - Comments', 'liquidfolio'), __('1 - Comment', 'liquidfolio'), __('% - Comments', 'liquidfolio'), 'comments'); echo "</p>";} ?>
							</div>
						</li>
						<?php $shown_post++;
					endif;?>
	...
	function form($instance)
	{
		...
		<div>
			<p>
				<label for="<?php echo $this->get_field_id(self::TITLE); ?>"><?php _e('Title:', 'liquidfolio'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id(self::NUMBER); ?>"><?php _e('Number of posts to show:', 'liquidfolio'); ?></label>
	...
	private function getDefaultFieldValues()
	{
		$default_values = array(
					self::TITLE		=> __('Popular posts', 'liquidfolio'),
					self::NUMBER	=> 5,
					);
