<?php
	...
	$gallery_terms = get_terms( Custom_Posts_Type_Gallery::TAXONOMY, array(    'hide_empty' => 0,    'fields' => 'ids') );
	$gallery_cat = (get_post_meta(get_the_ID(), SHORTNAME . '_gallery_cat', true)) ? get_post_meta(get_the_ID(), SHORTNAME . '_gallery_cat', true) :$gallery_terms;
	/**
	 * get page size for gallery thumbnails
	 */

	if(get_post_meta(get_the_ID(), SHORTNAME . Custom_Thumbnail_Gallery::META_SIZE_TYPE, true) == 'custom')
	{
		$page_width = get_post_meta(get_the_ID(), SHORTNAME . Custom_Thumbnail_Gallery::META_PAGE_WIDTH, true);
		$page_height = get_post_meta(get_the_ID(), SHORTNAME . Custom_Thumbnail_Gallery::META_PAGE_HEIGHT, true);
	}
	else
	{
...
//---------------------------------------------
		...
		if(get_post_meta(get_the_ID(), SHORTNAME . '_show_gallery_title_type', true) == 'custom')
		{
			$isShowingTitle = (bool) get_post_meta(get_the_ID(), SHORTNAME . '_show_gallery_title', true);
		}
		else
		{
			$isShowingTitle = (bool) get_post_meta($current_page_ID, SHORTNAME . '_show_title_on_page', true);
		}
...
//---------------------------------------------
		...
		{
			// Page value
			$lightBoxType = get_post_meta($current_page_ID, SHORTNAME . '_page_lightbox_type', true);
		}
...
?>
	<article <?php post_class($term_id.' gallery_listing') ?> id="post-<?php the_ID(); ?>" <?php echo $article_style;?> data-wm="<?php echo $multi_width?>" data-hm="<?php echo $multi_height?>" data-show_title="<?php echo $showTitle; ?>" <?php echo ($lightBoxType !== 'permalink')?$lightboxData: '';?>>
        <a href="<?php the_permalink(); ?>"    style="background:transparent"  class="double_thumb loading" data-pagew="<?php echo $page_width;?>" data-pageh="<?php echo $page_height; ?>" data-galleryid="<?php echo get_the_ID(); ?>" data-gallery_page_id="<?php echo $current_page_ID;?>">
