<?php
	...
	{
		$referer = preg_replace('/https?:\/\/' . $_SERVER['HTTP_HOST']. '\/?((index|preview)\.php)?/', '', $_SERVER['HTTP_REFERER']);

		if (!preg_match('/\/lightbox$/', $referer))
		{
			if ($rewrite)
			{
				$referer = "/$referer";
			}
			setcookie('koken_referer', $referer, 0, '/');
		}
		else if (isset($_COOKIE['koken_referer']))
		{
			$referer = $_COOKIE['koken_referer'];
		}
	}
	else
	{
	...
	if ($draft && !isset($_COOKIE['koken_session']))
	{
	...
	else
	{
		...
		$url = preg_replace('/^' . $regex . '|([\?&].*$)/', '', rtrim(urldecode($raw_url), '/'));
	}

	if (preg_match('~/__rewrite_test$~', $url))
	{
		die('koken:rewrite');
	}
	...
	if (!$draft && !isset($_GET['default_link']))
	{
		...
		{
			$cache_url = $base_path . preg_replace('/\?|&|=/', '_', preg_replace('/\?|&_pjax=[^&$]+/', '', urldecode(rtrim($cache_url, '/'))));
			$css = false;
		}

		$cache_path = $root_path . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'site' . str_replace('/', DIRECTORY_SEPARATOR, $cache_url) . ( $css ? '' : ( $pjax ? '.phtml' : '.html' ) );
	...
	{
		die( file_get_contents(Koken::$fallback_path . $ds . 'error' . $ds . 'json.html') );
	}

	Koken::$site = $site_api;
	Koken::$profile = Koken::$site['profile'];
	Koken::$location['defaults'] = Koken::$site['default_links'];
	...
	if (isset(Koken::$site['navigation']))
	{
		foreach(Koken::$site['navigation']['items'] as &$n)
		{
			...
			{
				Koken::$navigation_home_path = $n['path'];
			}
		}
	...
	// Create routes array from page_type description
	foreach(Koken::$site['routes'] as $arr)
	{
		$tail = $ds . $arr['template'] . '.lens';
		// TODO: May need to cache template tree so we don't kill things with IO
		if ($arr['template'] && ( file_exists(Koken::$template_path . $tail) || file_exists(Koken::$fallback_path . $tail) ))
		{
			$routes[$arr['path']] = array(
	...
	$routes[Koken::$site['default_links']['content_lightbox']] =
	$routes[Koken::$site['default_links']['content_in_album_lightbox']] = array(
		'template' => 'lightbox',
		'source' => false,
		'filters' => false,
		'vars' => false
	...
	$routes[Koken::$site['default_links']['album_lightbox']] = array(
		'template' => '___redirect_album_lightbox',
		'source' => false,
		'filters' => false,
		'vars' => false
	);

	if (!isset($routes['/albums/:id']))
	{
		$routes['/albums/:id'] = array(
			'template' => Koken::$site['default_templates']['album']
		);
	}

	$routed_variables = array();

	if ($url === '/' && Koken::$navigation_home_path)
	{
	...
	if (isset($final_path))
	{
		if ($final_path === '___redirect_album_lightbox')
		{
			$lb_curl = curl_init();
			curl_setopt($lb_curl, CURLOPT_URL, $protocol . '://' . $_SERVER['HTTP_HOST'] . $base_folder . '/api.php/albums/' . $routed_variables['id'] . '/content/limit:1');
			curl_setopt($lb_curl, CURLOPT_HEADER, 0);
			curl_setopt($lb_curl, CURLOPT_RETURNTRANSFER, 1);

			$album = json_decode( curl_exec($lb_curl), true );
			curl_close($lb_curl);

			$url = $base_folder . ( !$rewrite ? '/' . ( $draft ? 'preview' : 'index' ) . '.php' : '' ) . Koken::$site['default_links']['content_in_album_lightbox'];
			$url = str_replace(':album_id', $routed_variables['id'], $url);
			$url = str_replace(':id', $album['content'][0]['id'], $url);
			header("Location: $url");
			exit;
		}
		...
		else
		{
			...
			{
				$path = preg_replace_callback('/\{(site\.)?settings\.([^\}]+)\}/', 'parse_replacements', $matches[1]);
			...
			function go($tmpl, $pass = 1)
			{
				...
				else
				{
					...
					$pulse_str .= '<script src="' . Koken::$location['real_root_folder'] . join('"></script><script src="' . Koken::$location['real_root_folder'], $pulse_srcs) . '"></script>';
					$js = <<<JS
...
<script src="{$location['real_root_folder']}/app/site/themes/common/js/koken.js"></script>
<script>\$K.location = $location_json;\$K.theme = '{$site['theme']['path']}';\$K.retinaEnabled = $hdpi;</script>
<link href="{$location['real_root_folder']}/app/site/themes/common/css/mediaelement/mediaelementplayer.min.css" rel="stylesheet">
<script src="{$location['real_root_folder']}/app/site/themes/common/js/mediaelement-and-player.min.js"></script>
<link rel="alternate" type="application/atom+xml" title="{$site['title']}: All uploads" href="{$location['real_root_folder']}/feed/content/recent.rss" />
<link rel="alternate" type="application/atom+xml" title="{$site['title']}: Essays" href="{$location['real_root_folder']}/feed/essays/recent.rss" />
$pulse_str
JS;
				}
...
JS;
				...
				preg_match_all('/<\!\-\- KOKEN HEAD BEGIN \-\->(.*?)<!\-\- KOKEN HEAD END \-\->/s', $contents, $headers);
				$contents = preg_replace('/<\!\-\- KOKEN HEAD BEGIN \-\->(.*?)<!\-\- KOKEN HEAD END \-\->/s', '', $contents);
				...
				{
					$contents = preg_replace('/<title>.*?<\/title>/s', '', $contents);
				...
				$contents = preg_replace('/<koken_title>.*?<\/koken_title>/s', '', $contents);

				$header_str .= "\n<!--[if IE]>\n<script src=\"" . Koken::$location['real_root_folder'] . "/app/site/themes/common/js/html5shiv.js\"></script>\n<![endif]-->\n";

				if (strpos($contents, '<head>'))
				{
					preg_match('/<head>(.*)?<\/head>/s', $contents, $header);
					if (count($header))
					{
						$head = $header[1];
						preg_match_all('/<script.*?<\/script>/s', $head, $head_js);
						$head = preg_replace('/\s*<script.*?<\/script>\s*/s', '', $head) . "\n$header_str\n$js\n" . join("\n", $head_js[0]);
						$contents = preg_replace('/<head>(.*)?<\/head>/s', "<head>\n$head\n</head>", $contents);
					}
