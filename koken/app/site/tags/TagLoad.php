...
DOC;
			...
			if ($options['archive'])
			{
				...
				{
					case 'tag':
						$archive = "{$main}['archive'] = array('title' => str_replace(',', ', ', urldecode(Koken::\$routed_variables['id'])), 'type' => array('singular' => 'Tag', 'plural' => 'Tags'));";
						break;

					case 'category':
						$archive = "{$main}['archive'] = array('title' => {$main}['category']['title'], 'type' => array('singular' => 'Category', 'plural' => 'Categories'));";
						break;

					case 'date':
						$archive = "{$main}['archive'] = array('month' => isset(Koken::\$routed_variables['month']) ? Koken::\$routed_variables['month'] : false, 'year' => Koken::\$routed_variables['year'], 'title' => !isset(Koken::\$routed_variables['month']) ? Koken::\$routed_variables['year'] : date('F Y', strtotime(Koken::\$routed_variables['month'] . '/1/' . Koken::\$routed_variables['year'])), 'type' => array('singular' => 'Date', 'plural' => 'Dates'));";
						break;
				}
			}
...
DOC;
	...
	if (isset({$main}['album']) || isset({$main}['context']['album']))
	{
		...
		echo '<!-- KOKEN HEAD BEGIN --><link rel="alternate" type="application/atom+xml" title="' . Koken::\$site['title'] . ': Uploads from ' . \$__album['title'] . '" href="' . Koken::\$location['root_folder'] . '/feed/albums/' . \$__album['id'] . '/recent.rss" /><!-- KOKEN HEAD END -->';
	}
DOC;
			}
