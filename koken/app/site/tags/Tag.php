<?php
	...
	class Tag {
		...
		public function attr_parse($val, $wrap = false)
		{
			...
			{
				$o = preg_replace('/\{\{\s*([a-z_\.]+)\s*\}\}/', '" . Koken::out(\'$1\') . "', $val);
				if ($wrap)
				{
					return '<?php echo "' . $o. '" ?>';
				}
		...
		public function field_to_keys($param, $variable = 'value', $token_index = false)
		{
			...
			foreach($bits as $param)
			{
				...
				preg_match('/^(site|location|profile|source|settings|routed_variables|page_variables|pjax)/', $p, $global_matches);
...
DOC;
		...
		function load_params($source_options = false)
		{
				...
				foreach($defaults['filters'] as $filter)
				{
					if (strpos($filter, '=') !== false)
					{
						...
						else
						{
							...
							if (strpos($bits[0], 'category') === 0 && !is_numeric($bits[1]))
							{
			...
			$paginate = $options['paginate_from_url'] && $options['list'];
			...
			if ($options['id_from_url'] || $options['id'])
			{
				...
				{
					$url .= "/{$options['id_prefix']}\" . urlencode(Koken::\$routed_variables['id']) . \"";
				}
				else if ($options['id'])
				{
				...
				{
					$url .= '/context:" . (isset(Koken::$routed_variables[\'album_id\']) ? Koken::$routed_variables[\'album_id\'] : "stream") . "';
				}
