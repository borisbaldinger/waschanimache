<?php
	...
	class TagLink extends Tag {
		...
		function generate()
		{
			...
			{
				$this->parameters['url'] = $this->parameters['to'] === 'front' ? '"/"' : 'Koken::$site[\'default_links\'][\'' . $this->parameters['to'] . '\']';
				foreach($this->parameters as $key => $val)
				{
					if (strpos($key, 'filter:') === false) continue;
					$clean_key = str_replace('filter:', '', $key);
					unset($this->parameters[$key]);
					$this->parameters['url'] = "str_replace(':$clean_key', urlencode('" . $val . "'), " . $this->parameters['url'] . ')';
			...
			else if (isset($this->parameters['data']))
			{
				...
				if (preg_match('/(next|previous)_page$/', $this->parameters['data'], $data_matches))
				{
					$this->parameters['rel'] = substr($data_matches[1], 0, 4);
				}
...
DOC;
	...
	\$__url = strpos(\$__url, '/') === 0 ? Koken::\$location['root'] . \$__url  . ( Koken::\$preview ? (strpos(\$__url, '?') === false ? '?' : '&amp;') . 'preview=' . Koken::\$preview : '' ) : \$__url;
DOC;
