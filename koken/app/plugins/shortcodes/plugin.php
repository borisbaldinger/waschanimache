...
class BD_Shortcodes extends KokenPlugin {
	...
	function koken_media($attr)
	{
		...
		if (isset($attr['link']) && $attr['link'] !== 'none')
		{
			...
			{
				$link_pre = '<koken:link to="album" filter:id="' . $attr['album'] . '">';
				$link_post = '</koken:link>';
			}
			else
			{
...
<div class="k-content-embed">
	<koken:load source="content" filter:id="{$attr['id']}">
		<div class="k-content">
			$link_pre
			<koken:$tag />
			$link_post
...
HTML;
	...
	{
		$text = '';
		$img = '<img class="k-media-img" style="max-width:100%" src="' . $attr['_relative_root'] . 'storage/custom/' . $attr['filename'] . '" />';
