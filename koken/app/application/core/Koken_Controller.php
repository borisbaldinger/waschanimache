...
class Koken_Controller extends CI_Controller {
	...
	function __construct()
    {
		...
		if ($this->method === 'get')
		{
			if (preg_match('/^\/sessions/', $path_info))
			{
				$this->cache_path_to_file = false;
			}
			else if (file_exists($this->cache_path_to_file) && ENVIRONMENT === 'production')
			{
				...
				if ($mtime > $cache_stamp)
				{
					...
					die(file_get_contents($this->cache_path_to_file));
				}
			}
		}
		else
		{
			...
			if (!preg_match('/^\/sessions/', $path_info) && ENVIRONMENT === 'production')
			{
				touch($stamp);
	...
	function _output($data, $code = 200)
	{
		...
		if (isset($this->cache_path_to_file) && ENVIRONMENT === 'production' && $this->method === 'get' && (int) $code === 200)
		{
	...
	function check_for_rewrite()
	{
		...
		$rewrite_check = curl_exec($curl) === 'koken:rewrite';
		curl_close($curl);
