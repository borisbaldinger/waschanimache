...
class Content extends DataMapper {
	...
	function _format_tags()
	{
		...
		else
		{
			...
			$tags = preg_replace('/[^A-Za-z0-9\-\._\s,"]+/', '', $tags);
			...
			// One last cleanup
			$this->tags = strtolower(preg_replace('/\,+/', ',', preg_replace('/[^A-Za-z0-9\-_\.\s,]+/', '', $tags)));
	...
	{
		$base = FCPATH . DIRECTORY_SEPARATOR .
	...
	function _before()
	{
		...
		if ($this->file_type > 0)
		{
			include_once(FCPATH . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'koken' . DIRECTORY_SEPARATOR . 'ffmpeg.php');
			$ffmpeg = new FFmpeg($path);
			if ($ffmpeg->version())
			{
				// FFmpeg needs directory helper
				$this->duration = $ffmpeg->duration();
				if ($this->file_type == 2)
				{
					if ($ffmpeg->version_lame())
					{
						$length = $ffmpeg->waveform();
						if ($length)
						{
							$this->lg_preview = 'waveform.svg:50:50';
		...
		else
		{
			...
			if (isset($iptc['2#025']) && is_array($iptc['2#025']))
			{
				...
				{
					foreach($iptc['2#025'] as $key)
					{
						$words[] = '"' . $key . '"';
					}
				}

				$this->tags .= ' ' . join(',', $words);
			}
	...
	function _readify()
	{
		...
		{
			$this->tags = array_unique( explode(',', trim($this->tags, ',')) );
		}
		else
		{
			$this->tags = array();
		}
		$f = unserialize($this->focal_point);
		if (empty($f))
		{
			$f = array('x' => 50, 'y' => 50);
		}
	...
	{
		delete_files(FCPATH . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . $this->cache_path, true, 1);
	}
	...
	function to_array($options = array())
	{
		...
		$exclude = array('deleted', 'exif_make', 'exif_model', 'exif_iso', 'exif_camera_serial', 'exif_camera_lens', 'featured_order', 'favorite_order');
		...
		{
			$path = 'storage/originals/' . $this->path . '/' . $this->filename;
		...
		if (!isset($options['include_presets']) || $options['include_presets'])
		{
			...
			else
			{
				...
				$cache_base = $koken_url_info->base . (KOKEN_REWRITE ? 'storage/cache' : 'i.php') . '/' . str_replace('\\', '/', $this->cache_path) . '/' . $prefix;
		...
		if (array_key_exists('license', $data))
		{
			...
			@$exif = unserialize($this->exif);

			$data['geolocation'] = false;

			if (isset($exif['GPS']['GPSLatitude']))
			{
				include_once(FCPATH . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'koken' . DIRECTORY_SEPARATOR . 'gps.php');
