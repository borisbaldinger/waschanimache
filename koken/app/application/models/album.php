...
class Album extends DataMapper {
	...
	function context($params, $auth)
	{
		...
		$prev->where('deleted', 0)->where('left_id <', $this->left_id)->where('level', $this->level);
		$next->where('deleted', 0)->where('left_id >', $this->left_id)->where('level', $this->level);
		...
		if ($next_limit > 0)
		{
			$next->where('left_id >', $this->left_id)
		...
		if ($pre_limit > 0)
		{
			$prev->where('left_id <', $this->left_id)
				->where('level', $this->level)
		...
		else
		{
			...
			switch($method)
			{
				...
				case 'delete':
					...
					$u = new Album();
					$unlisted = $u->where('listed !=', $this->listed)->where_in('id', $ids)->get_iterated();
					foreach($unlisted as $u)
					{
						$u->listed = $this->listed;
						$u->save();
					}
					...
					{
						$d = new Album();
						$dest_copy = $d->select('level,left_id,right_id')->get_by_id($id);
						...
						if ($move_copy->left_id >= $destination_left)
						{
							$left += $size;
							$right += $size;
						}
						...
						$a->shift_tree_values($right + 1, -$size, $this->listed);
	...
	function to_array($options = array())
	{
		...
		{
			$this->audio_file = $koken_url_info->base . '/album-audio/' . $this->audio_file;
		}
		...
		if ($this->album_type == 1) {
			$this->smart_rules = unserialize($this->smart_rules);
		} else {
			$exclude[] = 'smart_rules';
		}

		$bools = array('smart', 'listed', 'featured');
		...
		$data['counts'] = array(
			...
			'audio' => (int) $this->audio_count,
			'images' => $this->total_count - $this->video_count - $this->audio_count
		);
		...
		foreach($this->covers->order_by("covers_{$this->db_join_prefix}albums_covers.id ASC")->get_iterated() as $f)
		{
			if ($f->exists())
			{
				$data['covers'][] = $f->to_array();
				$existing[] = $f->id;
			}
		}

		if ($this->album_type == 2 && $this->covers->result_count() < 3)
		{
			$a = new Album();
			$ids = $a->select('id')
						->where('right_id <', $this->right_id)
		...
		$data['site_link'] = array(
			'type' => 'album',
			'url' => $this->config->item('koken_url_info')->base . (KOKEN_REWRITE ? '' : 'index.php/') . 'albums/' . $data['id']
		);
