...
class System extends Koken_Controller {
	...
	function index()
	{
		...
		include(FCPATH . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'koken' . DIRECTORY_SEPARATOR . 'darkroom.php');
		include(FCPATH . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'koken' . DIRECTORY_SEPARATOR . 'ffmpeg.php');

		$ffmpeg = new FFmpeg;

		list($version, $imagick) = Darkroom::processing_library_version();
		if (!$version)
		{
		...
		{
			$processing_string = $version . ( $imagick ? ' via Imagick' : '');
		}
		...
		$data = array(
			...
			'imagemagick_support' => $processing_string,
			'ffmpeg_support' => $ffmpeg->version(),
			'upload_limit' => $max,
