...
class Update extends Koken_Controller {
	...
	function __construct()
    {
    	...
    	$this->migrate_path = FCPATH . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . 'migrations' . DIRECTORY_SEPARATOR;
        parent::__construct();
    }
    ...
    function theme()
    {
    	...
    	$themes = FCPATH . DIRECTORY_SEPARATOR . 'storage' .
    					DIRECTORY_SEPARATOR . 'themes';

    	$tmp_path = FCPATH . DIRECTORY_SEPARATOR . 'storage' .
    ...
    function migrate($n = false)
    {
    	...
    	if ($n === 'schema')
    	{
    		...
    		foreach($koken_tables as $table_name => $info)
    		{
    			...
    			if ($this->db->table_exists($table))
    			{
    				...
    				foreach($info['fields'] as $field_name => $field_info)
    				{
    					...
    					{
    						$field_info['type'] = preg_replace('/[^a-z_]/', '', strtolower($field_info['type']));
    						$diff = array_diff( $field_info, (array) $existing_fields[$field_name] );
    						if (!empty( $diff ))
    						{
			...
			$curl = curl_init();
			curl_setopt($curl, CURLOPT_URL, KOKEN_STORE_URL . '/register?domain=' . $_SERVER['HTTP_HOST'] . '&path=/' . $base_folder . '&uuid=' . $s->value . '&php=' . PHP_VERSION . '&version=' . KOKEN_VERSION . '&ip=' . $_SERVER['SERVER_ADDR']);
			...
			$themes_dir = FCPATH . DIRECTORY_SEPARATOR .
	...
	function index($v = false)
	{
		...
		function rollback($back)
		{
			foreach($back as $b)
			{
				$f = FCPATH . DIRECTORY_SEPARATOR . $b;
		...
		if ($v) {
			...
			$core = FCPATH . DIRECTORY_SEPARATOR . 'core.zip';
			...
			if ($this->_download($get_core, $core)) {
				...
				$movers = array('admin', 'app', 'api.php', 'i.php', 'index.php', 'preview.php', 'popup.php');
				$moved = array();
				...
				foreach($movers as $m) {
					$path = FCPATH . DIRECTORY_SEPARATOR . $m;
					$to = $path . '.off';
					if (file_exists($to))
					{
						exec("rm -rf $to");
