...
class Albums extends Koken_Controller {
	...
	function index()
	{
		...
		else
		{
			...
			if ($a->exists())
			{
				if ($a->listed == 0 && !$this->auth && is_numeric($id))
				{
	...
	function content()
	{
		...
		if ($album->exists())
		{
			if ($album->listed == 0 && !$this->auth && is_numeric($id))
			{
				$this->error('403', 'Private content.');
			}
		}
		...
		if ($album->level > 1)
		{
			...
			$final['album']['parent'] = $parent->to_array(array('auth' => $params['auth']));
		}
