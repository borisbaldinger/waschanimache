...
function create_htaccess($symlink = false)
{
	...
	$template = file_get_contents(FCPATH . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR . 'httpd' . DIRECTORY_SEPARATOR . '.htaccess');
	...
	$template = str_replace('%{DOCUMENT_ROOT}/storage', '%{DOCUMENT_ROOT}' . $base . '/storage', $template);
