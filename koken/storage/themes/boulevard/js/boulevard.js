$(function() {
	...
	if ($.support.pjax) {
		$(document).on('click', '#lane a, nav li:eq(0) a', function() {

			var self = $(this);

			// Write the album title or remove it
			if ( $(this).closest('#lane').length > 0 ) {
				$('hgroup h2').html( '/&nbsp;' + $(this).closest('.cell').find('h2 a').text().trim());
			} else {
				$('hgroup h2').html('');
			}
			//
	...
	var setupLaneHandler = function() {
		window.setTimeout(function() {
			$('#lane img')
				.css('cursor','pointer')
				.on('click', function() {
					if ( window.location.pathname.indexOf('albums') <= 0 ) { return true; }
				...
				.each(function(i,img) {
					if ( !this.complete ) {
						...
						$('#lane').css({
							// height: $('#lane img:eq(0)').css('height')
						});
					}
				});
